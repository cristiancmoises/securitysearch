# Direct upgrade: v0.9.30 → v0.9.41
...
The corrected v0.9.30 compatibility baseline is commit
`331cf550d8b279736126e7362fb699f99fc2bc66`, tree
`488b01ae481e8e4e95dad3743c2a175c43065c97`. Its historical
[baseline record](../data/upgrade-baseline-0.9.30.json) is preserved.
The current GitHub main read for this delivery is v0.9.40-r2 commit
`6d2a46f9300eb907b68bbcc92d5c774a30464ddc`, tree
`b0ab9966f02dd0c41ad8f936347b64cee5642c9f`.
Only GitHub was independently queried; the VPS and other remotes are not independently
verified by that read. Deployment uses `root@securityops.co:5119`.

## Files and paths

Save each launcher and matching `.sha256` directly in `~/Downloads`:
`securitysearch-v0.9.41-deploy-ionos.fish`,
`securitysearch-v0.9.41-publish-four-remotes.fish`, and
`securitysearch-audit-diagnostics-0.9.41.fish`.
...
3. Run `--audit-only`. It also prepares the commit when necessary, uploads/builds the
   candidate on IONOS, and requires all 124 native commands. It never replaces production.
...
with reset/force. Successfully prepared exact v0.9.41 is idempotent.
...
Atualize diretamente da v0.9.30 para a v0.9.41. Não instale versões intermediárias e não
...
124/0 e provedores reais antes da troca; o publicador separado só roda depois de
