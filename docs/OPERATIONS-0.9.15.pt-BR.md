# Security Search v0.9.15 — operação

Versão completa sem JavaScript no navegador, com marcador de assets **19**. Os botões Search Image, Search Pinterest e Search YouTube ficam abaixo de Search. Binternet, Invidious, seis layouts, qualidade alta, formatos disponíveis em cada provedor e os links/rodapé solicitados são mantidos.
...
`sh scripts/test.sh` executa regressões PHP/Python e HTTP local com provedor simulado. Não faz buscas externas. Consulte a [auditoria](AUDIT-0.9.15.md) e o [guia completo em inglês](OPERATIONS-0.9.15.md) para evidências e limites.
