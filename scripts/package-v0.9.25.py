#!/usr/bin/env python3
"""Build immutable source assets and an incremental recovery bundle from v0.9.25.
...
def source_archive(repo, commit):
    ...
    required = {'README.md', 'README.pt-BR.md', 'data/release-version.txt',
                ...
                'scripts/publish-v0.9.25.py', 'scripts/package-v0.9.25.py',
...
def package(repo, outdir):
    ...
    files = {archive_name: source, archive_name + '.sha256': checksum,
             ...
             'RELEASE-0.9.25.md': git(repo, 'show', commit + ':docs/RELEASE-0.9.25.md')}
    manifest = {'schema': 1, 'version': VERSION, 'asset_version': 29, 'commit': commit,
    ...
    spec = importlib.util.spec_from_file_location('release_publisher', repo / 'scripts/publish-v0.9.25.py')
    publisher = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(publisher)
    publisher.prepare(repo, outdir, repo / 'docs/RELEASE-0.9.25.md')
...
def main():
    ...
    return package(repo, args.output or repo.parent / 'securitysearch-release-v0.9.25')
