...
the clean source checkout: python3 scripts/publish-v0.9.20.py --assets /path/to/release-assets
"""
import argparse
import getpass
import hashlib
    ...
    try:
        for host, owner, kind in HOSTS:
            ...
            require(token and all(32 < ord(c) < 127 for c in token),
                    "All four tokens are required, without whitespace/control characters. No remote write started.")
            clients.append(API(host, owner, kind, token))
        failed = []
        print("Read-only preflight of all four repositories, refs, release notes and existing assets.", flush=True)
        for api in clients:
            try:
