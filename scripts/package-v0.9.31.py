#!/usr/bin/env python3
"""Build immutable source assets and an incremental recovery bundle from v0.9.31.
...
def source_archive(repo, commit):
    ...
    required = {'README.md', 'README.pt-BR.md', 'data/release-version.txt',
                ...
                'docs/RELEASE-0.9.31.md', 'docs/PERFORMANCE-0.9.31.md', 'docs/AUDIT-0.9.31.md', 'docs/AUDITFIX-0.9.20-r1.md',
                'scripts/publish-v0.9.31.py', 'scripts/package-v0.9.31.py',
                ...
                'scripts/delivery-profile.php', 'scripts/delivery-profile.py', 'docs/PERFORMANCE-0.9.31.md',
                'lib/google_cse_protocol.php', 'tests/google-flow-regression.php', 'tests/google-live-gate-regression.py', 'tools/secops-web-benchmark-v3.fish', 'docs/screenshots/securitysearch-0.9.31-home.png', 'docs/screenshots/securitysearch-0.9.31-mobile.png',
...
def package(repo, outdir):
    ...
    files = {archive_name: source, archive_name + '.sha256': checksum,
             ...
             'RELEASE-0.9.31.md': git(repo, 'show', commit + ':docs/RELEASE-0.9.31.md')}
    manifest = {'schema': 1, 'version': VERSION, 'asset_version': 35, 'commit': commit,
    ...
    spec = importlib.util.spec_from_file_location('release_publisher', repo / 'scripts/publish-v0.9.31.py')
    publisher = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(publisher)
    publisher.prepare(repo, outdir, repo / 'docs/RELEASE-0.9.31.md')
...
def main():
    ...
    return package(repo, args.output or repo.parent / 'securitysearch-release-v0.9.31')
