...
spec = importlib.util.spec_from_file_location("publisher", Path(__file__).resolve().parents[1] / "scripts/publish-v0.9.21.py")
...
class PackageTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("README.md", "README.pt-BR.md", "data/release-version.txt",
                     "docs/RELEASE-0.9.21.md", "docs/AUDITFIX-0.9.20-r1.md",
            ...
            target.write_text("0.9.21" if name.endswith("release-version.txt") else "fixture")
        for name in ("scripts/publish-v0.9.21.py", "scripts/package-v0.9.21.py"):
            target = self.repo/name
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_bytes((source/name).read_bytes())
        self.g("add", ".")
        self.g("-c", "commit.gpgsign=false", "commit", "-m", "release fixture")
        self.g("-c", "tag.gpgsign=false", "tag", "-a", "v0.9.21", "-m", "fixture tag")
        spec = importlib.util.spec_from_file_location("packager", source/"scripts/package-v0.9.21.py")
    ...
    def test_package_checksum_provenance_bundle_and_repeat(self):
        ...
        self.g('bundle','verify',str(self.output/'securitysearch-v0.9.21.bundle'))
        p.prepare(self.repo,self.output,self.repo/'docs/RELEASE-0.9.21.md')
    ...
    def test_dirty_checkout_has_no_tag_or_package_side_effect(self):
        before=self.g('rev-parse','refs/tags/v0.9.21')
        (self.repo/'README.md').write_text('private local changes')
        with self.assertRaises(self.builder.Failure):
            self.builder.package(self.repo,self.output)
        self.assertEqual(before,self.g('rev-parse','refs/tags/v0.9.21'))
