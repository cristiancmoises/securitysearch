...
class Contract(unittest.TestCase):
 def test_previous_85_commands_preserved_in_order(self):
  ...
  self.assertEqual(len(old),85);self.assertEqual(new[:85],old);self.assertEqual(len(new),119);self.assertEqual(len(new),len(set(new)))
 ...
 def test_critical_runtime_unchanged(self):
  ...
  for path,expected in hashes.items():self.assertEqual(hashlib.sha256((R/path).read_bytes()).hexdigest(),expected,path)
