...
class Home(unittest.TestCase):
 ...
 def test_lcp_image_is_early_eager_and_sized(self):
  ...
  self.assertLess(text.index('rel="preload"'),text.index('<style'));self.assertIn('?v41',img['src'])
 ...
 def test_custom_and_image_themes_still_supported(self):
  for theme in ['Custom','Tron','Lain','SecOps','Art','Gentoo']:
   with self.subTest(theme=theme):
    ...
    self.assertTrue(any('/themes/'+theme+'.css?v41' in l.get('href','')for l in p.links))
