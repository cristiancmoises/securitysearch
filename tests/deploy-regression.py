...
for failure in [None,'version','theme','theme_asset','script','csp','image_csp','image_asset','motion_asset','adapters']:
 responses=['20|Tron' if failure=='version' else ('23|Lain' if failure=='theme' else '28|Black'),
  'In Code We Trust. zupt-web.securityops.co '+('/static/themes/Lain.css?v28' if failure=='theme_asset' else '/static/themes/Black.css?v28')+('<script src="x"></script>' if failure=='script' else ''),
...
print('PASS: candidate gates validate asset 28, effective Black, homepage no-script CSP, scoped image CSP and enhancer asset.')
