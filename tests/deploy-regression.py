...
for mode in ['success','readiness_failure','cleanup_failure','lost_candidate_create','lost_rename','lost_replacement_create','offline_audit_failure','live_audit_failure','news_audit_failure']:
 ...
 with tempfile.TemporaryDirectory() as temp,patch.object(m,'BACKUP_ROOT',Path(temp)/'backups'),patch.object(m,'LOCK_PATH',str(Path(temp)/'lock')),patch.object(m,'api',side_effect=engine.api),patch.object(m,'run',side_effect=run),patch.object(m,'healthy',side_effect=engine.healthy),patch.object(m,'offline_audit',side_effect=RuntimeError('Audit failed') if mode=='offline_audit_failure' else None),patch.object(m,'live_news_gate',return_value=('bing','en-US'),side_effect=RuntimeError('News gate failed') if mode=='news_audit_failure' else None),patch.object(m,'verify_news_config'),patch.object(m,'live_binternet_gate',side_effect=RuntimeError('Live gate failed') if mode=='live_audit_failure' else None),patch.object(m.os,'geteuid',return_value=0),patch.object(m.shutil,'which',return_value='/fixture/program'),patch.object(m.subprocess,'run',return_value=subprocess.CompletedProcess([],1)):
  ...
  if mode=='success':
   ...
   assert 'FOURGET_NEWS_RSS_PRIMARY=bing' in migrated_env and 'FOURGET_DEFAULT_SCRAPER_NEWS=newswire' in migrated_env
  else:
...
for failure in [None,'version','theme','theme_asset','script','csp','image_csp','image_asset','motion_asset','adapters']:
 responses=['20|Tron' if failure=='version' else ('23|Lain' if failure=='theme' else '29|Black'),
  'In Code We Trust. zupt-web.securityops.co '+('/static/themes/Lain.css?v29' if failure=='theme_asset' else '<style data-home-style="base"></style><style data-home-style="black"></style><style data-home-style="controls"></style>' )+('<script src="x"></script>' if failure=='script' else ''),
...
print('PASS: candidate gates validate asset 29, effective Black, homepage no-script CSP, scoped image CSP and enhancer asset.')
