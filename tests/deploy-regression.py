...
for failure in [None,'version','theme','fpm_runtime','theme_asset','script','csp','image_csp','image_asset','motion_asset','adapters']:
 responses=['20|Tron' if failure=='version' else (f'{m.ASSET_VERSION}|Lain' if failure=='theme' else f'{m.ASSET_VERSION}|Black'),
  ...
  'In Code We Trust. zupt-web.securityops.co '+('/static/themes/Lain.css?v42' if failure=='theme_asset' else '<style data-home-style="base"></style><style data-home-style="black"></style><style data-home-style="controls"></style>' )+('<script src="x"></script>' if failure=='script' else ''),
