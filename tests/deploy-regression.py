...
for mode in ['success','readiness_failure','cleanup_failure','lost_candidate_create','lost_rename','lost_replacement_create']:
 ...
 with tempfile.TemporaryDirectory() as temp,patch.object(m,'BACKUP_ROOT',Path(temp)/'backups'),patch.object(m,'LOCK_PATH',str(Path(temp)/'lock')),patch.object(m,'api',side_effect=engine.api),patch.object(m,'run',side_effect=run),patch.object(m,'healthy',side_effect=engine.healthy),patch.object(m.os,'geteuid',return_value=0),patch.object(m.shutil,'which',return_value='/fixture/program'),patch.object(m.subprocess,'run',return_value=subprocess.CompletedProcess([],1)):
  ...
  if mode=='success':
   ...
   assert 'FOURGET_DEFAULT_THEME=Tron' in migrated_env and 'FOURGET_SERVER_NAME=original' in migrated_env
...
for failure in [None,'version','theme','theme_asset','script','csp','image_csp','image_asset','motion_asset','adapters']:
 responses=['20|Tron' if failure=='version' else ('23|Lain' if failure=='theme' else '23|Tron'),
  'In Code We Trust. zupt-web.securityops.co '+('/static/themes/Lain.css?v23' if failure=='theme_asset' else '/static/themes/Tron.css?v23')+('<script src="x"></script>' if failure=='script' else ''),
...
print('PASS: candidate gates validate asset 23, effective Tron, homepage no-script CSP, scoped image CSP and enhancer asset.')
