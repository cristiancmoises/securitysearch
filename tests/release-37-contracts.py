...
class Release37(unittest.TestCase):
 def test_all_106_previous_commands_remain_in_order(self):
  ...
  self.assertEqual(len(old),106);self.assertEqual(new[:106],old);self.assertEqual(len(new),124);self.assertEqual(len(set(new)),124)
 ...
 def test_release_advances_without_asset_or_provider_change(self):
  ...
  for name in ('scripts/deploy-ionos.py','scripts/package-v0.9.41.py'):self.assertIn('ASSET_VERSION = 41',(R/name).read_text())
