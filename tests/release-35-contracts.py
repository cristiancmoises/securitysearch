...
class Release35(unittest.TestCase):
 def test_previous_commands_preserved(self):
  ...
  self.assertEqual(len(old),97);self.assertEqual(new[:97],old);self.assertEqual(len(new),130);self.assertEqual(len(new),len(set(new)))
 def test_version_and_asset(self):
  self.assertEqual((R/'data/release-version.txt').read_text().strip(),'0.9.42');self.assertIn('const VERSION = 42;',(R/'data/config.php').read_text());self.assertIn('ASSET_VERSION = 42',(R/'scripts/deploy-ionos.py').read_text());self.assertIn('ASSET_VERSION = 42',(R/'scripts/package-v0.9.42.py').read_text())
