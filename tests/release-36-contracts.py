...
class Release36(unittest.TestCase):
 def test_previous_commands_preserved(self):
  ...
  self.assertEqual(len(old),102);self.assertEqual(new[:102],old);self.assertEqual(len(new),124);self.assertEqual(len(set(new)),124)
 def test_version_and_asset(self):
  self.assertEqual((R/'data/release-version.txt').read_text().strip(),'0.9.41')
  self.assertIn('const VERSION = 41;',(R/'data/config.php').read_text())
  for p in ('scripts/deploy-ionos.py','scripts/package-v0.9.41.py'):self.assertIn('ASSET_VERSION = 41',(R/p).read_text())
