...
class Release36(unittest.TestCase):
 def test_previous_commands_preserved(self):
  ...
  self.assertEqual(len(old),102);self.assertEqual(new[:102],old);self.assertEqual(len(new),119);self.assertEqual(len(set(new)),119)
 def test_version_and_asset(self):
  self.assertEqual((R/'data/release-version.txt').read_text().strip(),'0.9.40')
  self.assertIn('const VERSION = 40;',(R/'data/config.php').read_text())
  for p in ('scripts/deploy-ionos.py','scripts/package-v0.9.40.py'):self.assertIn('ASSET_VERSION = 40',(R/p).read_text())
