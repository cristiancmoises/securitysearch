...
spec = importlib.util.spec_from_file_location("publisher", Path(__file__).resolve().parents[1] / "scripts/publish-v0.9.40.py")
...
class PackageTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ('docs/INDEX.md', 'docs/UPGRADE-0.9.30-to-0.9.40.md', 'docs/OPERATIONS-0.9.40.md', 'docs/OPERATIONS-0.9.40.pt-BR.md', 'docs/PUBLISHING.md', 'docs/PUBLISHING.pt-BR.md', 'docs/TROUBLESHOOTING-0.9.40.md', 'docs/RELEASE.md', 'data/upgrade-baseline-0.9.30.json', 'README.pt-BR.md', 'tests/search-state-regression.php', 'tests/search-state-http-regression.py', 'tests/release-40-contracts.py', 'tests/runtime_history.py', 'tests/native_config_fixture.py', 'docs/AUDITFIX-0.9.40-r1.md', 'docs/AUDITFIX-0.9.40-r2.md', 'data/search-state-changes-0.9.40.json', 'data/audit-commands-0.9.38.json', "tests/audit_fixture.py", "data/audit-baseline-0.9.37.json", "data/audit-commands-0.9.40.json", "scripts/audit_evidence.py", "scripts/audit_stream.py", "tests/audit-evidence-regression.py", "tests/audit-stream-regression.py", "tests/audit-only-regression.py", "tests/release-38-contracts.py", "scripts/operator_payload.py", "tests/operator-payload-regression.py", "tests/http_runtime.py", "tests/http-runtime-regression.py", "tests/release-37-contracts.py", "data/audit-baseline-0.9.36.json", "data/preserved-runtime-0.9.38.json", "README.md", "README.pt-BR.md", "data/release-version.txt",
            ...
            target.write_text("0.9.40" if name.endswith("release-version.txt") else "fixture")
        for name in ("scripts/publish-v0.9.40.py", "scripts/package-v0.9.40.py", "scripts/release_media_policy.py"):
            target = self.repo/name
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_bytes((source/name).read_bytes())
        self.g("add", ".")
        self.g("-c", "commit.gpgsign=false", "commit", "-m", "release fixture")
        self.g("-c", "tag.gpgsign=false", "tag", "-a", "v0.9.40", "-m", "fixture tag")
        spec = importlib.util.spec_from_file_location("packager", source/"scripts/package-v0.9.40.py")
    ...
    def test_package_checksum_provenance_bundle_and_repeat(self):
        ...
        self.assertEqual(manifest['asset_version'],40)
        self.assertEqual(manifest['commit'],self.g('rev-parse','HEAD').decode().strip())
        self.g('bundle','verify',str(self.output/'securitysearch-v0.9.40.bundle'))
        p.prepare(self.repo,self.output,self.repo/'docs/RELEASE-0.9.40.md')
    ...
    def test_dirty_checkout_has_no_tag_or_package_side_effect(self):
        before=self.g('rev-parse','refs/tags/v0.9.40')
        (self.repo/'README.md').write_text('private local changes')
        with self.assertRaises(self.builder.Failure):
            self.builder.package(self.repo,self.output)
        self.assertEqual(before,self.g('rev-parse','refs/tags/v0.9.40'))
