#!/usr/bin/env fish
# SecOps TTFB Benchmark 4.0.0 — GNU Guix / Fish entry point.
# Run with: fish --no-config "$HOME/Downloads/secops-web-benchmark-v4.fish"
...
"""SecOps TTFB Benchmark 4.0: sequential, auditable HTTP homepage measurements.
...
DEFAULT_UA = "SecOpsWebBenchmark/4.0 (curl; HTTP measurement; no JavaScript)"
...
def classify(meta, exit_code, headers, body, parse_error=""):
    ...
    if float(meta["time_starttransfer"]) <= 0 or float(meta["time_total"]) <= 0 or float(meta["time_starttransfer"]) > float(meta["time_total"]):
...
def provenance(meta, status_chain, site):
    """Do not rank a different origin or use interim headers as final-response TTFB."""
    host = urlsplit(str(meta.get("url_effective", ""))).hostname
    if host not in (site, "www." + site):
        return "origin_mismatch", "final origin differs from this named target"
    if any(type(code) is int and 100 <= code < 200 for code in status_chain):
        return "interim_response", "informational response: curl first-byte timing is not final-document TTFB"
    return "eligible", "origin and final-response timing provenance checked"
...
def measure(curl, site, url, args, env, proxies, work):
    ...
    if status == "eligible":
        provenance_status, provenance_reason = provenance(meta, chain, site)
        if provenance_status != "eligible":
            status, reason = provenance_status, provenance_reason
...
def leaderboard(rows, metadata):
    ...
    seen = set()
    for row in rows:
        value = number(row.get("ttfb_ms"))
        if (row.get("phase") == "measured" and row.get("status") == "eligible"
                and row.get("site") in samples and value is not None and value > 0
                and number(row.get("total_ms")) is not None and number(row.get("total_ms")) >= value):
            key = (row["site"], row["round"])
            if key in seen:
                raise ValueError("Duplicate site/round measurement")
            seen.add(key)
    ...
    for site, _ in SITES:
        ...
        items.append({"site": site, "rank": None, "status": status,
                      ...
                      "matched": matched, "median_ms": statistics.median(ttfbs) if ttfbs else None,
                      "html_median_ms": total,
                      "ttfb_median_ms": statistics.median(ttfbs) if ttfbs else None,
                      "p95_ms": percentile(ttfbs),
    ...
    if comparable:
        detail = (f"Lowest median TTFB: {ranked[0][\'median_ms\']:.3f} ms; "
    ...
    return {"criterion": "lowest median TTFB time on matching eligible rounds",
...
LEADER_HEADERS = ("Rank", "Site", "OK/Plan", "TTFB ms", "HTML ms", "TTFB p95 ms", "Slower %", "Result")
...
def leaderboard_cells(board):
    ...
    for item in board["items"]:
        output.append((str(item["rank"]) if item["rank"] is not None else "-", item["site"],
                       ...
                       fmt(item["html_median_ms"], 3), fmt(item["p95_ms"], 3),
...
def html_report(board, text, metadata):
    ...
    *{box-sizing:border-box}body{margin:0;padding:32px 22px 56px}main{max-width:1240px;margin:auto}
    ...
    return ("<!doctype html><html lang=\\"en\\"><head><meta charset=\\"utf-8\\">"
            ...
            \'<h1>Homepage TTFB leaderboard</h1><p class="muted">Seven targets. Measured from this machine and network, at this time.</p></header>\'
...
def report(rows, out, metadata):
    ...
    lines += ["", "Method and limits:",
              ...
              "Ranking: median TTFB (curl time_starttransfer), on matching eligible rounds.",
              "Full HTTPS GETs; new connections, not guaranteed cold DNS/TLS caches. Interim responses excluded.",
              "Same machine/network/time only. Not a global, all-search-engines, or search-results ranking.",
...
def self_test():
    ...
    class Tests(unittest.TestCase):
        ...
        def test_winner_and_sort(self):
            ...
            self.assertEqual(board["items"][1]["slower_percent"], 80)
            self.assertIn("44.4% lower time", board["detail"])
        ...
        def test_ties_are_shared_not_alphabetical_winners(self):
            ...
            for row in rows:
                if row["site"] == "duckduckgo.com":
                    row["total_ms"], row["ttfb_ms"] = 100, 50
        ...
        def test_matched_ranking_removes_unshared_round(self):
            ...
            for r in rows:
                if r["site"] == "google.com" and r["round"] == 7:
                    r["total_ms"], r["ttfb_ms"] = 999999, 999990
            b = leaderboard(rows, {"state": "completed", "runs": 7})
            self.assertEqual(b["common_rounds"], list(range(1, 7)))
            self.assertTrue(all(i["samples_used"] == 6 for i in b["items"]))
            self.assertEqual(next(i for i in b["items"] if i["site"] == "google.com")["p95_ms"], 90)
        ...
        def test_ttfb_and_total_have_different_winners(self):
            rows = self.ranking_rows()
            for r in rows:
                if r["site"] == "4get.ca": r.update(total_ms=90, ttfb_ms=80)
                elif r["site"] == "securityops.co": r.update(total_ms=500, ttfb_ms=20)
            board=leaderboard(rows, {"state":"completed", "runs":7})
            self.assertEqual(board["winners"],["securityops.co"])
            self.assertEqual(board["items"][0]["html_median_ms"],500)
            self.assertEqual(board["items"][0]["median_ms"],20)
            self.assertIn("TTFB",board["criterion"])

        def test_missing_zero_negative_or_nonfinite_ttfb_cannot_win(self):
            for invalid in (None, 0, -1, float("nan"), float("inf")):
                rows=self.ranking_rows()
                for r in rows:
                    if r["site"] == "securityops.co": r["ttfb_ms"]=invalid
                self.assertNotIn("securityops.co",leaderboard(rows,{"state":"completed","runs":7})["winners"])

        def test_duplicate_measurements_refused(self):
            rows=self.ranking_rows();rows.append(dict(rows[0]))
            with self.assertRaises(ValueError):leaderboard(rows,{"state":"completed","runs":7})

        def test_cross_origin_document_not_this_target(self):
            self.assertEqual(provenance({"url_effective":"https://www.intermotors.pl/"},[200],"securityops.co")[0],"origin_mismatch")
            self.assertEqual(provenance({"url_effective":"https://securityops.co.attacker.invalid/"},[200],"securityops.co")[0],"origin_mismatch")

        def test_www_redirect_same_service_allowed(self):
            self.assertEqual(provenance({"url_effective":"https://www.google.com/"},[301,200],"google.com")[0],"eligible")

        def test_early_hints_cannot_win_final_document_ranking(self):
            self.assertEqual(provenance({"url_effective":"https://securityops.co/"},[103,200],"securityops.co")[0],"interim_response")

        def test_equal_ttfb_co_winners_even_with_different_body_times(self):
            rows=self.ranking_rows()
            for r in rows:
                if r["site"]=="4get.ca":r.update(ttfb_ms=50,total_ms=999)
            self.assertEqual(set(leaderboard(rows,{"state":"completed","runs":7})["winners"]),{"securityops.co","4get.ca"})

        def test_ttfb_cannot_exceed_total_even_if_rows_are_injected(self):
            rows=self.ranking_rows()
            for r in rows:
                if r["site"]=="securityops.co":r.update(ttfb_ms=10,total_ms=1)
            self.assertNotIn("securityops.co",leaderboard(rows,{"state":"completed","runs":7})["winners"])
